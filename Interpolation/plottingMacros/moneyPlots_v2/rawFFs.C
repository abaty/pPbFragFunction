...
{ 
  ...
  for(int i = 0; i<10; i++)
  {
    ...
    hist[i]->Draw("same");
    if(i<5) tlat->DrawLatex(0.8,0.000004,Form("%d < p_{T}^{jet} < %d GeV/c",(int)FF_Bound[i],(int)FF_Bound[i+1]));
    if(i==0) tlat->DrawLatex(3,1.5,"5.3 pb^{-1} (2.76 TeV pp)");
    if(i==1) tlat->DrawLatex(20,1.5,"ak3PF jets");
    if(i==2) tlat->DrawLatex(20,1.5,"|#eta_{CM}^{jet}|<1.5");
    if(i==4) tlat->DrawLatex(8,1.5,"CMS Preliminary");
    if(i==5) tlat->DrawLatex(4,1.5,"2.5 fb^{-1} (7 TeV pp)");
...
//    if(i==5) tlat->DrawLatex(0.8,0.000002,"#int_{7 TeV pp}Ldt = 2.5 fb^{-1}");
    if(i==0)
    {
      TLegend * leg = new TLegend(0.3,0.2,0.8,0.4);
