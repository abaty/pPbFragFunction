...
void getComparison(int mode)
{
  ...
  TFile * infpPb = new TFile("../FragmentationFunctions_ChargeMinusUE3.root","read"); 
  TFile * infPbp = new TFile("../FragmentationFunctions_ChargePlusUE3.root","read");
 ...
 // leg->AddEntry((TObject*)0, "140 < p_{t}^{jet} < 200 GeV/c", "");
  ...
  if(mode==3) leg->AddEntry((TObject*)0,"2.76 TeV reco PYTHIA","");
  ...
  leg->AddEntry(pPbFF[0], "- charged FF", "p");
  leg->AddEntry(PbpFF[0], "+ charged FF", "p");
  ...
  PbpFF[i]->GetYaxis()->SetTitle("FF_{+}/FF_{-}");
  ...
  c2->SaveAs(Form("../plots/Charge_Comparison_%s.png",files[mode]));
  c2->SaveAs(Form("../plots/Charge_Comparison_%s.pdf",files[mode]));
  delete l;
  delete lat;
  delete c2;
  infpPb->Close();
