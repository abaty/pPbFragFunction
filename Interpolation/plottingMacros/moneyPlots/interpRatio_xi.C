...
{  
  ...
  for(int i = 0; i<15; i++)
  {
    ...
    else if(i<10) hist[i] = (TH1D*)inf1->Get(Form("pPb5Pbp5TeV_data_interp_%d_%d_xi",(int)FF_Bound[i%5],(int)FF_Bound[i%5+1]));
  ...
  for(int i = 0; i<15; i++)
  {
    ...
    {
      sysU[i] = (TH1D*)inf2->Get(Form("interp_TotUP%d_xi",i%5+5));
      sysD[i] = (TH1D*)inf2->Get(Form("interp_TotDOWN%d_xi",i%5+5));
    }    
    else 
    { 
      sysU[i] = (TH1D*)inf2->Get(Form("FFratio_TotUP%d_xi",i%5+5));
      sysD[i] = (TH1D*)inf2->Get(Form("FFratio_TotDOWN%d_xi",i%5+5));
    }
  ...
  for(int i = 0; i<15; i++)
  {
    ...
    if(i==0)
    {
      TLegend * leg = new TLegend(0.35,0.18,0.5,0.5);
  ...
  c->SaveAs(Form("../../plots/prettyInterpRatio_UE%d_xi.png",UEtype));
  c->SaveAs(Form("../../plots/prettyInterpRatio_UE%d_xi.pdf",UEtype));
}
